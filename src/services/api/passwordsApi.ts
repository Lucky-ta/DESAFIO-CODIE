...
export const createPassword = async (data: DataShape) => {
    try {
        const response = await AXIOS_API.post('/userPasswords', JSON.stringify(data), {
...
export const getAllPasswords = async () => {
    try {
        const response = await AXIOS_API.get('/userPasswords');
        return response.data;
    } catch (e: any) {
        return { message: e.message }
    }
...
export const updatePasswordById = async (passwordId: number, updatedData: DataShape) => {
    try {
        const response = await AXIOS_API.put(`/userPasswords/${passwordId}`, updatedData);
        return response.data;
    } catch (e: any) {
        return { message: e.message }
    }
